...
icon_path = os.path.join(BASE_DIR, 'icons', 'ridit.ico')
...
def ridit_price(url):
    ...
    if response.status_code == 200:
        ...
        try:
            price_tag = soup.find('span', attrs={'class': 'new_price notranslate break-word'}).text.strip()
            price = re.sub(r'[^0-9.]', '', price_tag)
        except AttributeError:
            price = None

        try:
            old_tag = soup.find('span', attrs={'class': 'old_price notranslate break-word'}).text.strip()
        ...
        try:
            image_tag = soup.find('img', attrs={'class': 'img'})
...
if __name__ == "__main__":
    ridit_price('https://ridit.com.ua/uk/p/1228710334-mongodb-v-deystvii-kayl-benker/')