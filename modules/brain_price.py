...
icon_path = os.path.join(BASE_DIR, 'icons', 'brain.ico')
...
def brain_price(url):
    ...
    if response.status_code == 200:
        ...
        product_name = soup.find('div', attrs={'id':'br-pr-1'}).find('h1').text.strip()

        try:
            price_tag = soup.find('div', attrs={'class': 'br-pr-np'}).find('span', attrs={'class': 'red-price'}).text.strip()
        ...
        try:
            image_tag = soup.find('img', attrs={'id': 'product_main_image'})
...
if __name__ == "__main__":
    brain_price('https://brain.com.ua/ukr/Televizor_Vinga_S24HD25B-p1054579.html')