...
icon_path = os.path.join(BASE_DIR, 'icons', 'telemart.ico')
...
def telemart_price(url):
    response = requests.get(url, headers=headers, cookies=cookies)
    if response.status_code == 200:
        soup = BeautifulSoup(response.text, 'html.parser')
        product_name = soup.find('h1', attrs={'class':'card-block__title'}).text.strip()

        try:
            price_tag = soup.find('div', attrs={'class': 'card-block__price-summ'}).text.strip()
            price = re.sub(r'[^0-9.]', '', price_tag)
        except AttributeError:
            price = None

        try:
            old_tag = soup.find('div', attrs={'class': 'card-block__price-old'}).text.strip()
        ...
        try:
            image_tag = soup.find('div', attrs={'class': 'img4zoom'})
            image_url = image_tag.get('data-url')
            if image_url:
...
if __name__ == "__main__":
    telemart_price('https://telemart.ua/ua/products/lenovo-ideapad-slim-5-16iah8-83bg004sra-cloud-grey/')